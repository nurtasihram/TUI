...
void ListBox::_OnTouch(const PidState *pState) {
	if (pState) {
		if (pState->Pressed == 0)
...
void ListBox::_OnMouseOver(const PidState *pState) {
	if (!pOwner)
		return;
	if (pState) {
...
void ListBox::_Callback(WM_MSG *pMsg) {
	auto pObj = (ListBox *)pMsg->pWin;
	...
	case WM_TOUCH:
		...
		return;
	case WM_MOUSEOVER:
		pObj->_OnMouseOver((const PidState *)pMsg->data);
		return;
	case WM_DELETE:
		pObj->~ListBox();
		break;
	case WM_KEY:
	{
		auto ki = (const WM_KEY_INFO *)pMsg->data;
		if (ki->PressedCnt > 0)
