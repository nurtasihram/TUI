...
static void _OnPaint(ListView *pObj, WM_MSG *pMsg) {
	...
	SRect rClip = *(const SRect *)pMsg->data - pObj->Rect().left_top();
...
static void _OnTouch(ListView *pObj, WM_MSG *pMsg) {
	int Notification;
...
static void _LISTVIEW_Callback(WM_MSG *pMsg) {
	ListView *pObj = (ListView *)pMsg->pWin;
...
void LISTVIEW_SetSel(ListView *pObj, int NewSel) {
	...
	pObj->NotifyParent(WM_NOTIFICATION_SEL_CHANGED);
}
