...
static void _OSK_updateShiftCaps(void) {
	...
	for (i = 0; i < GUI_COUNTOF(_aButtonData); ++i) {
		...
		if (inf->type == K_CHAR)
			...
			if (FnSel) {
				BUTTON_SetText(inf->pButton, inf->acLabel2 + 2);
				continue;
			}
			m = Shift;
		}
			...
			continue;
		BUTTON_SetText(inf->pButton, m ? inf->acLabel2 : inf->acLabel1);
	}
}
...
static void _OSK_createButtons(WObj *pWin, Point size) {
	...
	for (int i = 0; i < GUI_COUNTOF(_aButtonData); i++) {
		ButtonInfo *inf = _aButtonData + i;
		Button *pButton = inf->pButton = BUTTON_CreateEx(
			_OSK_ceil(inf->pxPos * size.x), _OSK_ceil(inf->pyPos * size.y),
			_OSK_ceil(inf->pWidth * size.x), _OSK_ceil(inf->pHeight * size.y),
			pWin, WC_VISIBLE, 0, ID_BUTTON + i);
		BUTTON_SetText(pButton, inf->acLabel1);
...
static void _OSK_onKeyV(int Id) {
	ButtonInfo *inf = _aButtonData + Id - ID_BUTTON;
	uint8_t ab = 0;
	int c = 0;
	switch (inf->type) {
	case K_CHAR_FN:
...
static void _OSK_onKeyH(int Id) {
	ButtonInfo *inf = _aButtonData + Id - ID_BUTTON;
	uint8_t ab = 0;
	int c = 0;
	switch (inf->type) {
...
void (*_OSK_onKey)(int Id) = _OSK_onKeyV;
static void _OSK_callback(WM_MESSAGE *pMsg) {
	WObj *pWin = pMsg->pWin;
	switch (pMsg->MsgId) {
	case WM_CREATE:
		GUI.PenColor(GUI_GRAY);
		break;
	case WM_PAINT:
		GUI.PenColor(GUI_GRAY);
		GUI_FillRect(0, 0, pWin->SizeX(), pWin->SizeY());
		break;
	case WM_NOTIFY_PARENT:
		...
		break;
	case WM_SIZE:
		_OSK_updateButtons(WM_GetClientWindow(pWin), pWin->Size());
		break;
	}
}
...
void OSK_Show() {
	pKeypad = Frame::Create(
		0, 120, 320, 120,
		WM_GetDesktopWindow(),
		WC_VISIBLE | WC_STAYONTOP | WC_NOACTIVATE,
		0, 0, "Keyboard",
		_OSK_callback);
	_OSK_createButtons(WM_GetClientWindow(pKeypad), pKeypad->Size());
...
void Show_Keypad() {
	auto pFrame = Frame::Create(
		0, 0, 320, 240,
		WM_GetDesktopWindow(), WC_VISIBLE,
		0, 0, "Notepad", 0);
	auto pMult = MULTIEDIT_CreateEx(
		0, 0, 0, 0,
		WM_GetClientWindow(pFrame),
		WC_VISIBLE, 0,
		GUI_ID_MULTIEDIT0, 100, nullptr);
	pFrame->Moveable(true);
	pFrame->ClientColor(GUI_WHITE);
	pFrame->AddCloseButton(FRAMEWIN_BUTTON_RIGHT, 0);
	MULTIEDIT_SetWrapWord(pMult);
	MULTIEDIT_SetText(pMult, "123");
	pMult->Focus();
